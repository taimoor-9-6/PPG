...
Original file is located at
    https://colab.research.google.com/drive/1UBGaLv2n0iLmEoY3kT7Tknmym7vj20lI
"""
...
from tensorflow.keras.layers import  Dense, Dropout,  Input, Reshape, Conv1D, Activation,BatchNormalization, MaxPooling1D,LSTM,Add,Flatten
...
#Function to determine breathing rate
    ...
    #   m=Conv1D(32,kernel_size=40,padding='same')(i)
    #   m=BatchNormalization()(m)
    #   m=Activation('relu')(m)
    #   m=MaxPooling1D(pool_size=2)(m)
    #   m=Dropout(0.1)(m)
    #   m=Conv1D(32,kernel_size=40,padding='same')(m)
    #   m=BatchNormalization()(m)
    #   m=Activation('relu')(m)
    #   m=MaxPooling1D(pool_size=4)(m)
    #   m=Dropout(0.1)(m)
    #   # m=MaxPooling1D(pool_size=34)(m)
...
def ResNet():    
      i=Input(shape=(INPUT_SHAPE,1),name="input")
      i_skip=i
      m=Conv1D(30,kernel_size=40,padding='same')(i)
      m=BatchNormalization()(m)
      m=Activation('relu')(m)
...
def plotHistory(histories, name, metric):
    ...
    plt.plot(x_axis, histories.history['loss'], label=metric+'_'+name)
    plt.scatter(x_axis, histories.history['loss'])
...
plt.title('Synthetic Dataset Loss without U-Net Architecture')
plt.ylabel('Loss')
plt.xlabel('Epoch')
...
with open('drive/My Drive/RAship/bidmc_resnet_loss.csv', 'w') as f:
  loss_1.to_csv(f)
...
with open('drive/My Drive/RAship/bidmc_resnet_preds_true_temp.csv', 'w') as f:
  bidmc.to_csv(f)
